...
const AboutUs = () => {
  ...
  return (
    <section className="bg-[#7b071e] overflow-x-hidden overflow-y-hidden text-white py-16 px-4">
      <ScrollTrigger onEnter={() => setScrolling(true)} onExit={() => setScrolling(false)}>
        <div className="flex flex-col lg:flex-row gap-8 items-center justify-between">
          {/* Masked Image Section */}
          <div className={`relative w-full lg:w-1/2 mb-12 lg:mb-0 transform transition-all duration-700 ${scrolling ? 'translate-x-0 opacity-100' : '-translate-x-10 opacity-0'}`}>
          ...
          {/* Text Content */}
          <div className={`w-full lg:w-1/2 mt-0 transform transition-all duration-700 ${scrolling ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'}`}>
            <svg
              id="icon"
