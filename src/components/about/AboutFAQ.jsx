            ...
            <div className="px-10 md:lg:px-16">
                <div className="grid grid-cols-1 md:lg:grid-cols-2 ">
                    <div className="pt-3 w-9/12">
                        <h1 className="text-base text-[#7b071e] md:lg:text-6xl">Got questions? Were here to help!</h1>
                    </div>
                    <div>
                        {faq.map((faq, index) => (
                            <div className="px-1" key={faq.id}>
                                <div
                                    ...
                                    className={` mb-6 w-full overflow-hidden p-5 rounded-md transition-colors duration-300 ${openFAQ === faq.id ? "bg-gradient-to-br from-amber-300 to-yellow-500" : "bg-gradient-to-br from-amber-300 to-yellow-500"}`}
                                >
                                    <div
                                        className="flex cursor-pointer items-start justify-between "
                                        onClick={() => toggleFAQ(faq.id)}
                                    >
                                        <p className={`text-sm md:lg:text-lg font-bold transition-colors duration-300 ${openFAQ === faq.id ? "text-black" : "text-black"}`}>{faq.title}</p>
                                        <div className=" relative ml-10 mt-1 flex h-5 w-5 items-center justify-center">
                                            <div
                                                className={`absolute h-5 w-0.5 transition-transform duration-300 ${openFAQ === faq.id ? "rotate-90 bg-red-600" : "bg-black"}`}
                                            ></div>

                                            <div className={`h-0.5 w-5  ${openFAQ === faq.id ? "" : "bg-black"}`}></div>
                                        </div>
                                    </div>
                                    {openFAQ === faq.id && (
                                        <div className=" w-full overflow-hidden mb-4 max-w-2xl lg:max-w-4xl">
                                            <p className={`text-sm md:lg:text-base pt-3 transition-colors duration-300 ${openFAQ === faq.id ? "text-black" : "text-black"}`}>{faq.content}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
